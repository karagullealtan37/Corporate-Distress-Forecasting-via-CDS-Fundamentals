"""
EXPERIMENT 1B: Reduce XGBoost Overfitting with Stronger Regularization

Goal: Train XGBoost models with different regularization levels to reduce
      the train/test gap while maintaining test performance.

Strategy:
    1. Load same train/test data as base pipeline
    2. Train multiple XGBoost variants with increasing regularization
    3. Compare train vs test performance
    4. Identify best trade-off

Outputs:
    - Models: output/experiments/models/*.pkl
    - Results: output/experiments/xgboost_overfitting_experiment.csv
    - Figures: report/figures/experiments/xgboost_overfitting_comparison.png
"""
...
def train_xgboost_variants(X_train, y_train, X_test, y_test):
    """
    Train multiple XGBoost variants with different regularization levels.
    """
    print_section("TRAINING XGBOOST VARIANTS")
    ...
    for i, config in enumerate(configs, 1):
        ...
        # Save model
        model_file = EXP_MODELS_DIR / f"xgboost_{config['name'].lower().replace(' ', '_')}.pkl"
...
def plot_results(results_df):
    ...
    bars1 = ax1.bar(x - width/2, results_df['train_auc'], width, label='Train AUC', 
                    color='steelblue', alpha=0.8)
    bars2 = ax1.bar(x + width/2, results_df['test_auc'], width, label='Test AUC', 
                    color='darkorange', alpha=0.8)
    
    # Add value labels on bars
    for bar, val in zip(bars1, results_df['train_auc']):
        height = bar.get_height()
        ax1.text(bar.get_x() + bar.get_width()/2., height,
                f'{val:.4f}',
                ha='center', va='bottom', fontsize=8, fontweight='bold')
    
    for bar, val in zip(bars2, results_df['test_auc']):
        height = bar.get_height()
        ax1.text(bar.get_x() + bar.get_width()/2., height,
                f'{val:.4f}',
                ha='center', va='bottom', fontsize=8, fontweight='bold')
    ...
    ax1.set_title('XGBoost: Train vs Test AUC', fontsize=13, fontweight='bold', pad=10)
    ax1.set_xticks(x)
    ...
    # Color coding: Green = good (low gap), Orange = moderate, Red = bad (high gap)
    # Gap < 0.1 (10%) = GREEN, Gap 0.1-0.2 = ORANGE, Gap > 0.2 (20%) = RED
    colors = []
    ...
    ax2.set_title('XGBoost: Overfitting Gap Analysis (Green=Good, Red=Bad)', fontsize=13, fontweight='bold', pad=10)
    ax2.set_xticks(x)
    ax2.set_xticklabels(results_df['model'], rotation=45, ha='right', fontsize=9)
    ax2.grid(True, alpha=0.3, axis='y')
    ax2.axhline(y=0.1, color='green', linestyle='--', alpha=0.5, linewidth=1.5, label='10% threshold (good)')
    ax2.axhline(y=0.2, color='red', linestyle='--', alpha=0.5, linewidth=1.5, label='20% threshold (bad)')
    ax2.legend(fontsize=8)
    
    # Plot 3: Test AUC vs Overfitting Gap (Scatter)
    ax3 = fig.add_subplot(gs[1, 0])
    
    # Assign explicit colors based on gap thresholds
    scatter_colors = []
    for gap in results_df['auc_gap']:
        if gap < 0.1:  # < 10% gap = GREEN (good)
            scatter_colors.append('green')
        elif gap < 0.2:  # 10-20% gap = ORANGE (moderate)
            scatter_colors.append('orange')
        else:  # > 20% gap = RED (bad)
            scatter_colors.append('red')
    
    scatter = ax3.scatter(results_df['auc_gap'], results_df['test_auc'], 
                         s=200, c=scatter_colors,
    ...
    ax3.set_title('XGBoost: Trade-off Analysis (Green=Good, Red=Bad)', fontsize=13, fontweight='bold', pad=10)
    ax3.grid(True, alpha=0.3)
    
    # Add legend for color coding
    from matplotlib.patches import Patch
    legend_elements = [
        Patch(facecolor='green', alpha=0.7, label='Low Gap (<10%)'),
        Patch(facecolor='orange', alpha=0.7, label='Moderate Gap (10-20%)'),
        Patch(facecolor='red', alpha=0.7, label='High Gap (>20%)')
    ]
    
    # Optimal region (high test AUC, low gap)
    ax3.axvline(x=0.1, color='green', linestyle='--', alpha=0.5, linewidth=2)
    ax3.axhline(y=0.63, color='blue', linestyle='--', alpha=0.5, linewidth=2)
    ax3.text(0.05, 0.635, 'Optimal\nRegion', fontsize=9, color='darkgreen', 
            fontweight='bold', ha='center', bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.5))
    ax3.legend(handles=legend_elements, fontsize=8, loc='lower right')
    
    # Plot 4: F1 Score Comparison
    ax4 = fig.add_subplot(gs[1, 1])
    
    bars3 = ax4.bar(x - width/2, results_df['train_f1'], width, label='Train F1', 
                    color='steelblue', alpha=0.8)
    bars4 = ax4.bar(x + width/2, results_df['test_f1'], width, label='Test F1', 
                    color='darkorange', alpha=0.8)
    
    # Add value labels on bars
    for bar, val in zip(bars3, results_df['train_f1']):
        height = bar.get_height()
        ax4.text(bar.get_x() + bar.get_width()/2., height,
                f'{val:.4f}',
                ha='center', va='bottom', fontsize=8, fontweight='bold')
    
    for bar, val in zip(bars4, results_df['test_f1']):
        height = bar.get_height()
        ax4.text(bar.get_x() + bar.get_width()/2., height,
                f'{val:.4f}',
                ha='center', va='bottom', fontsize=8, fontweight='bold')
    ...
    ax4.set_title('XGBoost: Train vs Test F1 Score', fontsize=13, fontweight='bold', pad=10)
    ax4.set_xticks(x)
    ...
    # Save
    output_file = EXP_FIGURES_DIR / 'xgboost_overfitting_comparison.png'
...
def generate_report(results_df):
    ...
    # Save results
    results_file = EXP_OUTPUT_DIR / 'xgboost_overfitting_experiment.csv'
    results_df.to_csv(results_file, index=False)
    print(f"✓ Saved results: {results_file}\n")
    
    # Print comparison table
    print("XGBOOST REGULARIZATION RESULTS:")
    print("-" * 100)
    ...
    if best_tradeoff['auc_gap'] < 0.15:
        print("\n✅ SUCCESS: Found XGBoost configuration with low overfitting (<15% gap)")
        ...
        print("\n⚠️  MODERATE: Best XGBoost configuration still has 15-20% gap")
        print(f"   → Use: {best_tradeoff['model']}")
        print(f"   → Consider: This is acceptable for complex models")
        print(f"   → Alternative: Try LightGBM or simpler model")
    else:
        print("\n⚠️  HIGH OVERFITTING: All XGBoost configurations show >20% gap")
        print(f"   → Best option: {best_tradeoff['model']}")
        print(f"   → Consider: LightGBM, Logistic Regression, or more data")
        print(f"   → Note: Temporal degradation suggests data quality issues")
...
def main():
    """
    Main execution: XGBoost overfitting reduction experiment.
    """
    print("\n" + "="*80)
    print("EXPERIMENT 1B: REDUCE XGBOOST OVERFITTING".center(80))
    print("="*80)
    ...
    # Train variants
    results, models = train_xgboost_variants(X_train, y_train, X_test, y_test)
    ...
    print("\nNext: Compare XGBoost vs LightGBM regularization results\n")
